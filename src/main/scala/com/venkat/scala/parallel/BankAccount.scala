...
object BankAccount extends App{
  ...
  var cnt = 0
  Future{for(i<- 1 to 100000000) cnt += 1}.foreach{_ => println("f1 done")}
  Future{for(i<- 1 to 100000000) cnt += 1}.foreach{_ => println("f2 done")}
  Thread.sleep(1000)
  println(cnt)
  ...
  Future{ useBuffers(b2, b1)}.foreach{_ => println("Call 2 done. ")} // thread dead lock
  Thread.sleep(2000)
  println("Main done!")
}
